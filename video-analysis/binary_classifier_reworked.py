...
from bc_audio.utils import generate_train_val_test_idx, calculate_random_baseline_from_splits, generate_train_val_split, generate_clean_train_and_val_idx, get_metrics, get_clean_foldwise_df
...
MODELS = {
        ...
        "RF": {'function': RandomForestClassifier(), 'param_grid': [{'RF__n_estimators': [2,5,10,20],'RF__criterion': ['gini', 'entropy', 'log_loss'], 'RF__max_depth': [1,2,3,4,5,10,15,20],
...
def main():
    ...
    for cam_position in cam_positions:
        ...
        experiment_savepath = DIR_PATH / f'inference/sklearn/experiment-{experiment}'
        ...
        for features in FEATURE_LIST:
            ...
            for K in [5]:
                ...
                logging.info(f"Feature set: {features}\n")

                print(f"Feature set: {features}\n")
                
                extracted_data = extract_from_raw_data(features, summary_stat = 'mean', position=cam_position)

                logging.info(f'{cam_position}-cam analysis!!! \n')

                logging.info(f'Featureset: {features} , Number: {feat_counter} \n')

                _, val_idx, _, val_pair_ids = generate_train_val_split(extracted_data, SEED, sklearn=True, k = K)
                ...
                for i in range(0,len(val_pair_ids)):
                    ...
                    test_idx_kfold = extracted_data.loc[ extracted_data["pair_id"].isin(test_pair_ids_kfold)].index.to_list()

                    validation_sliced_df = extracted_data.loc[ ~ extracted_data.index.isin(test_idx_kfold) ].reset_index(drop=True)

                    train_idx_kfold, val_idx_kfold, train_pair_ids_kfold  = generate_clean_train_and_val_idx(validation_sliced_df, extracted_data.pair_id.unique(), test_pair_ids_kfold, val_pair_ids_kfold) 
                    ...
                    train_rb, val_rb, test_rb = calculate_random_baseline_from_splits(extracted_data, train_pair_ids_kfold, val_pair_ids_kfold, test_pair_ids_kfold)

                    logging.info(f"Train pair IDs: {train_pair_ids_kfold}\n")

                    logging.info(f"Val pair IDs: {val_pair_ids}\n")
                    ...
                    X_test = extracted_data.loc[ test_idx_kfold][features]

                    Y_test = extracted_data.loc[ test_idx_kfold]['condition'].apply(lambda x: 1.0 if (x == "positive") else 0.0)
                    
                    print('Data extracted successfully!  \n')

                    model_savepath = experiment_savepath / f'model/fold-{K}/fold-{i}'
